...
class XBMCPrehrajme(xbmcprovider.XBMCMultiResolverContentProvider):
    ...
    def evalSchedules(self):
        if not self.scanRunning() and not self.isPlaying():
            ...
            util.info("PREHRAJ.ME Spustam co mam naplanovane")
            subs = self.get_subs()
            new_items = False
            for url, name in subs.iteritems():
                util.info("PREHRAJ.ME SUBS URL" + url)
                if xbmc.abortRequested:
                    util.info("PREHRAJ.ME Mam zdochnut!")
                    return
            ...
            util.info("PREHRAJ.ME Nieco srotuje, tak nic nerobim")
    ...
    def add_item(self, params):
        error = False
        arg = {"play": params['url'], 'cp': 'prehraj.me', "title": params['name']}
